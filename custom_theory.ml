...
module Make (Uf : Uf.S) (Use : Use.S with type r = Uf.R.r)
  ...
  with type uf = Uf.t
    ...
    let rec add_term (lt, env, cc, acc) (t, s, ex) =
      ...
      let (lt, inst, cc), forms = I.add_witness (lt, env.inst, cc) 
        (t, s, ex) in
      let env = { env with inst = inst } in
      (* Assume the resulting formulas *)
      let (lt, env, cc, acc) = List.fold_left assume_formula
    ...
    and assume_formula (lt, env, cc, acc) (f, ex) =
      if Boxed.Map.mem f env.seen then 
        ...
        with Not_found ->
          ...
          let (lt, env, cc, acc) =
            match Formula.view f.Boxed.formula, 
              Formula.view f.Boxed.view with
                  ...
                  if f.Boxed.polarity then
                    ...
                    if Options.debug_custom () then
                      (Format.fprintf Options.fmt "Custom Theory deduces: %a@."
                        Literal.LT.print lv;
                      Format.fprintf Options.fmt "with explanations : %a@."
                        Explanation.print ex);
                    let acc = (Sig.LTerm lv, ex) :: acc in
                    add_term ((Sig.LTerm lv, ex) :: lt, env, cc, acc) 
                      (terms, f.Boxed.subst, ex)
                  else 
                      ...
                      with Not_found ->
                        ...
                        let conseq = 
                          add_conseq (Boxed.mk_not f1) (f2, ex)
                            env.conseq in
                        (lt, { env with splits=splits; conseq=conseq },cc,acc))
                ...
                | Formula.Skolem {Formula.sko_subst = f_subst; sko_f = ff},
                  ...
                  (match Formula.view (Formula.mk_not f.Boxed.view) with
                    ...
                    | Formula.Lemma {Formula.qvars = vars; triggers = trs;
                      ...
                      let acc = assume_formula acc (ff, ex) in
    ...
    let rec clean lt (cc, env) =
      ...
      if la = [] then
        let (lt, inst, cc), la = I.get_instances (lt, env.inst, cc) in
        let env = { env with inst = inst } in
        if la = [] then (cc, Some env), lt
        else let (lt, env, cc, la) =
               List.fold_left assume_formula (lt, env, cc, []) la in
             let ((cc, env), lt) = assume_ann lt (cc, env) la in
             clean lt (cc, env)
    ...
    let assume_literal env lt l =
      ...
      match env with
        | None -> 
          let cc, lt = List.fold_left (fun (cc, lt) l ->
            match l with
              | Sig.LBoxed b, ex -> assert false
              | l, ex -> 
                let cc, (lt, _) = CC.assume_literal cc (lt, []) [l, ex] in
                cc, lt)  (cc, lt) l in
          to_env (cc, None), lt
        | Some env ->
    ...
    let add_theory env lt formulas =
      let cc, env = from_env env in
      let env = match env with
        | Some env -> env
        | None ->
          { splits = Boxed.Map.empty;
            conseq = Boxed.Map.empty;
            seen = Boxed.Map.empty;
            inst = I.empty cc } in
      let (lt, env, cc, lits) =
        List.fold_left (fun ((lt, env, cc, acc)) f ->
          assume_formula (lt, env, cc, acc) 
            (Boxed.from_formula f true, Explanation.empty)) 
          ((lt, env, cc, [])) formulas in
      let env, lt = assume_ann lt (cc, env) lits in
      let env, lt = clean lt env in
      to_env env, lt
    ...
    let add env lt a ex = 
      ...
      match env with
        ...
        | Some env ->
          ...
          let env, lt = assume_ann lt (cc, env) la in
          let env, lt = clean lt env in
          to_env env, lt
