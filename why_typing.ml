...
module Types = struct
  ...
  let fresh_vars vars loc =
    List.map
      (fun x -> 
	if MString.mem x !implicite_tyvars then error (TypeDuplicateVar x) loc;
        Ty.Tvar (Ty.fresh_var ())
      ) vars
  ...
  let add env vars id body loc = 
    ...
    let ty_vars = fresh_vars vars loc in
    match body with
      | Abstract -> 
  ...
  let add_logics env ac names pp_profile loc = 
	...
	| PPredicate args -> 
	  { args = List.map (Types.ty_of_pp loc env.types None) args; 
	    result = Ty.Tbool }
	(*| PFunction ([], PPTvarid (_, loc)) ->  error CannotGeneralize loc *)
	| PFunction(args, res) -> 
